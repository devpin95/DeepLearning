...
def lossPlot(loss):
    ...
    plt.plot(x, loss)
    plt.title('Big-CNN Epoch-Loss Plot', fontdict={'fontsize': 15}, pad=20)
    plt.xlabel('Epoch', fontdict={'fontsize': 11}, labelpad=20)
    plt.ylabel('Loss', fontdict={'fontsize': 11}, labelpad=20)
    plt.savefig('BigCNN-EpochLoss.png', bbox_inches='tight', pad_inches=0.5)
    plt.show()
...
def lossWallPlot(times, loss):
    ...
    plt.plot(times, loss)
    plt.title('Big-CNN Loss-Wall Plot', fontdict={'fontsize': 15}, pad=20)
    plt.xlabel('Time (seconds)', fontdict={'fontsize': 11}, labelpad=20)
    plt.ylabel('Loss', fontdict={'fontsize': 11}, labelpad=20)
    plt.savefig('BigCNN-LossWall.png', bbox_inches='tight', pad_inches=0.5)
    plt.show()
...
def confusionMatrix(cm):
    ...
    plt.title('Big-CNN Confusion Matrix', fontdict={'fontsize': 15}, pad=20)
    plt.xlabel('Predicted', fontdict={'fontsize': 11}, labelpad=20)
    plt.ylabel('True', fontdict={'fontsize': 11}, labelpad=20)
    plt.savefig('BigCNN-ConfusionMatrix.png', bbox_inches=None, pad_inches=0.5)
    plt.show()
...
def main():
    ...
    minibatch_size = 200
    epochs = 50
    conv = {'layer1': {'num_features': 48, 'feature_size': 3},
            'layer2': {'num_features': 96, 'feature_size': 3}}
    pool_size = 2
    ...
    # Set up the model
    model = Sequential([
        Conv2D(conv['layer1']['num_features'], conv['layer1']['feature_size'], input_shape=input_shape),
        MaxPooling2D(pool_size),
        Conv2D(conv['layer2']['num_features'], conv['layer2']['feature_size'], input_shape=input_shape),
        MaxPooling2D(pool_size),
        Flatten(),
